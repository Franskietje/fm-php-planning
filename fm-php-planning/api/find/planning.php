...
try 
{
  ...
  foreach ($records as $r)
  {
    ...
    $start = $fd['start_uur'] ?? null;
    $stop = $fd['stop_uur'] ?? null;

    if ($start) {
      $start = substr($start, 0, 5) . 'u.';
    }
    if ($stop) {
      $stop = substr($stop, 0, 5) . 'u.';
    }

    // Event opslaan
    $planning[$dossierId][] = ['type' => $fd['type'] ?? null, 'datum_van' => $van, 'datum_tot' => $tot, 'start_uur' => $start, 'stop_uur' => $stop];
